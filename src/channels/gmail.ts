...
export class GmailChannel implements Channel {
  ...
  private async processMessage(messageId: string): Promise<void> {
    ...
    const isSelfSent = senderEmail === this.userEmail;
    // For self-sent emails, only process those starting with "@Andy -" to avoid reply loops
    if (isSelfSent && !subject.startsWith('@Andy -')) return;
    ...
    // Always store under mainJid so the message loop (which only polls
    // registeredGroups) can find and process the message.
